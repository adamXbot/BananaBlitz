...
#   DIST_DIR — directory containing one or more BananaBlitz-<version>.dmg
...
if [[ -z "${SPARKLE_PRIVATE_KEY:-}" ]]; then
  ...
  echo "       BananaBlitz/Info.plist's SUPublicEDKey." >&2
  exit 2
fi
...
if ! command -v "$GENERATE_APPCAST" >/dev/null 2>&1 && [[ ! -x "$GENERATE_APPCAST" ]]; then
